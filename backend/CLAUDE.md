...
| `app/topics/` | Topic CRUD (`/api/topics`): PDF upload → extracted notes doc, 0-100 mastery score + memory, session history, per-topic `instructions` (user's standing guidance, injected into every agent call; legacy topics without the key are backfilled on read). `service.py` also holds the quiz-session state machine (`active_session` on the topic dict) |
